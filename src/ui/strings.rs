...
const TABLE: &[(&str, &str, &str)] = &[
    ...
    ("backend_dirty", "● 已实时同步到准星 · 点「应用」写入配置文件", "● Live-synced · hit Apply to save to disk"),
    ("backend_file_mode", "● 未检测到主程序 · 点「应用」写盘，主程序启动后自动加载", "● Main program not found · Apply writes to disk (auto-loaded when it starts)"),
    ...
    ("center_on_monitor", "准星移到该屏中心", "Center crosshair on this screen"),
    ("clear", "清除", "Clear"),
    ...
    ("duplicate_failed", "复制失败", "Duplicate failed"),
    ...
    ("hotkey_manual", "手动输入", "Type manually"),
    ("hotkey_next", "切换下一预设热键", "Next hotkey"),
    ("hotkey_note", "点输入框直接录制组合键（字母 / 数字 / F1-F24 / 方向键 / 符号键），也可手工输入 Ctrl+Shift+F1；保存后立即注册，被其它程序占用时不会生效", "Click the box to record a combo (letters / digits / F1-F24 / arrows / punctuation); you can also type Ctrl+Shift+F1. Registered right after saving, ignored if another app owns it"),
    ("hotkey_record", "点击录制", "Click to record"),
    ("hotkey_recording", "录制中：按下组合键（Esc 取消）", "Recording: press a combo (Esc cancels)"),
    ("hotkey_toggle", "切换准星热键（如 Ctrl+F1）", "Toggle hotkey"),
    ("image_choose", "选择图片…", "Choose image…"),
    ("image_none", "未选择图片（形状需设为「自定义图片」）", "No image selected (shape must be Custom Image)"),
    ("image_path", "图片路径", "Image path"),
    ("image_preview", "图片预览", "Image preview"),
    ("image_scale", "图片缩放", "Image scale"),
    ("image_unreadable", "无法读取该图片", "Cannot read this image"),
    ("import", "导入", "Import"),
    ...
    ("monitor", "显示器", "Monitor"),
    ("monitor_follow", "跟随前台窗口所在屏", "Follow the foreground window's screen"),
    ("monitor_primary", "主显示器", "Primary display"),
    ("monitor_refresh", "刷新显示器", "Refresh displays"),
    ("monitors_title", "显示器", "Displays"),
    ...
    ("outline_thickness", "描边厚度", "Outline width"),
    ("pos_x", "X", "X"),
    ("pos_y", "Y", "Y"),
    ...
    ("recover_ms", "恢复速度（ms/px）", "Recovery (ms/px)"),
    ("rename", "重命名", "Rename"),
    ("rename_confirm", "确认重命名", "Confirm rename"),
    ("rename_failed", "重命名失败（重名或名称非法）", "Rename failed (name taken or invalid)"),
    ...
    ("templates_hint", "点击缩略图套用", "Click a thumbnail to apply"),
    ("theme", "主题", "Theme"),
    ("theme_auto", "自动", "Auto"),
    ("theme_dark", "深色", "Dark"),
    ...
    ("trigger_threshold", "触发阈值（0-255）", "Threshold (0-255)"),
    ("window_close", "关闭窗口", "Close window"),
    ("window_maximize", "最大化 / 还原", "Maximize / restore"),
    ("window_minimize", "最小化", "Minimize"),
];
