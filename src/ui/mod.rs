...
//! 透感：**双模式底色** —— `windowfx::blur_active()` 为真（系统亚克力 / Win11 背板已生效）时走
//!       `Palette::translucent(level)`：基底 alpha 按玻璃强度在 74（最透）~150（较实）之间线性插值，
//!       其余玻璃层同比缩放，桌面能透过窗口被系统模糊；为假时用不透明底兜底（此时忽略强度，界面依旧完整）。
//!       毛玻璃模式与强度是应用级设置（`store.app.glass`），在「系统」分区调节、启动时恢复。
//!       玻璃"有厚度"另靠三件事：跟随鼠标的径向高光（整窗 + 每张面板）、光斑/面板的**反向视差**、
//!       按光照方向（光标侧亮、背面暗）的边缘折射带。
//! 动效：统一走 `*_and_easing`（cubic_out / expo_out / quadratic_out）—— 注意 egui 的 `animate_bool_with_time`
//!       内部是**线性**缓动，直接用会显得机械；时长按场景分级，动画收敛后停止重绘（静止零重绘）。
//! 性能：背板（基底 + 渐变 + 光斑 + 网格 + 噪点）**预烘焙成贴图**（缓存键含尺寸/主题/透底/强度，尺寸变化去抖），
//!       每帧只画 1 张贴图 + 少量光斑网格；面板阴影 6 层。
